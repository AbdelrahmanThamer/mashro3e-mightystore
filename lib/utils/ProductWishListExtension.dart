    ...
    return Observer(builder: (context) {
      return Container(
        ...
        child: wishListStore.isItemInWishlist(widget.mProductModel!.id!) == false
            ? Icon(Icons.favorite_border, color: isHalloween ? white : Theme.of(context).textTheme.subtitle2!.color, size: 16)
...
onClickProduct(BuildContext context, ProductResponse? mProductModel) async {
  ...
  if (getIntAsync(PRODUCT_DETAIL_VARIANT, defaultValue: 1) == 1) {
    result = await ProductDetailScreen1(mProId: mProductModel!.id).launch(context, pageRouteAnimation: PageRouteAnimation.Slide);
  } else if (getIntAsync(PRODUCT_DETAIL_VARIANT, defaultValue: 1) == 2) {
    result = await ProductDetailScreen2(mProId: mProductModel!.id).launch(context, pageRouteAnimation: PageRouteAnimation.Slide);
  } else if (getIntAsync(PRODUCT_DETAIL_VARIANT, defaultValue: 1) == 3) {
    result = await ProductDetailScreen3(mProId: mProductModel!.id).launch(context, pageRouteAnimation: PageRouteAnimation.Slide);
  } else {
    result = await ProductDetailScreen1(mProId: mProductModel!.id).launch(context, pageRouteAnimation: PageRouteAnimation.Slide);
  }
/*  if (result == null) {
